...
export default function DashboardRecentAppointments() {
  ...
  useEffect(() => {
    const updateTodaysList = () => {
      // 1. Fetch live data from localStorage
      const saved = localStorage.getItem("app_appointments");
      if (saved) {
        const allAppointments = JSON.parse(saved);
        
        // 2. Get the current date in YYYY-MM-DD format (matches your AppDashboard format)
        const todayStr = new Date().toISOString().split("T")[0];
        
        // 3. Filter for appointments matching exactly "Today"
        const todaysList = allAppointments.filter(appt => appt.date === todayStr);
        
        // 4. Sort by time so the earliest appointments appear first
    ...
    // Optional: Refresh data if the window regains focus (to catch updates made in other tabs)
      ...
      <div className="space-y-4">
        {appointments.length === 0 ? (
          ...
          appointments.map((appointment) => {
            const isSpecial = !!appointment.treatmentType;
            ...
            return (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 font-mono tracking-tight">PID: {appointment.patientId}</p>
                </div>
                
                <div className="flex items-center gap-6">
                  <div className="text-right hidden md:block">
                    <p className="text-sm font-bold text-slate-700">{appointment.time}</p>
                  ...
                  <div className="flex items-center">
                    <span className={`
                      min-w-[85px] text-center px-2.5 py-1 rounded text-[11px] font-bold uppercase tracking-wide
